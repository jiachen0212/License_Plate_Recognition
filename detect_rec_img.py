...
import time
from rec import plate_recogition  # self.lprnet.load_state_dict(torch.load('./my_lprnet_model.pth', map_location=self.device))
...
class license_plate_ocr:
    ...
    def det_rec(self, srcimg):
        ...
        for b in dets:
            ...
            cv2.imwrite('./img_box_jiaozheng.jpg', processed)

            # 输入lprnet识别模型, 得到车牌文本信息
            ...
            # 矫正后的landms
            cv2.circle(srcimg, (new_x1, new_y1), 2, (0, 0, 0), thickness=5)
            cv2.circle(srcimg, (new_x2, new_y2), 2, (0, 0, 0), thickness=5)
            cv2.circle(srcimg, (new_x3, new_y3), 2, (0, 0, 0), thickness=5)
            cv2.circle(srcimg, (new_x4, new_y4), 2, (0, 0, 0), thickness=5)
            # self.points_ref: np.float32([[0, 0], [94, 0], [0, 24], [94, 24]])
            cv2.circle(srcimg, (0, 0), 2, (0 , 255, 255), thickness=5)
            cv2.circle(srcimg, (94, 0), 2, (0, 255, 255), thickness=5)
            cv2.circle(srcimg, (0, 24), 2, (0, 255, 255), thickness=5)
            cv2.circle(srcimg, (94, 24), 2, (0, 255, 255), thickness=5)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgpath', default='imgs/2.jpeg', type=str, help='show detection results')
